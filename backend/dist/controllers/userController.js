...
import { getUserProfile, getUserPublicProfile, updateProfile, updateProfilePicture, getUserReputation, search, } from '../services/userService';
/**
...
export const getUser = async (req, res, next) => {
    try {
        ...
        if (!user) {
            res.status(404).json(createErrorResponse(new ApiError(404, 'NOT_FOUND', 'User not found'), req.path));
            return;
        }
        res.status(200).json(createSuccessResponse({ user }));
    }
...
export const searchUsers = async (req, res, next) => {
    try {
        ...
        const result = await search(q, parseInt(limit), parseInt(offset));
        res.status(200).json(createSuccessResponse(result));
    }
    catch (error) {
        if (error instanceof ApiError) {
