 ...
 */
export const findBookingsByGuestId = async (guestId, status, limit = 20, offset = 0) => {
    try {
 ...
 */
export const findBookingsByHostId = async (hostId, status, limit = 20, offset = 0) => {
    try {
 ...
 */
export const findBookingsByListingId = async (listingId, limit = 20, offset = 0) => {
    try {
        const result = await pool.query(`SELECT * FROM bookings 
       WHERE listing_id = $1 
       ORDER BY check_in_date DESC 
       LIMIT $2 OFFSET $3`, [listingId, limit, offset]);
        const countResult = await pool.query(`SELECT COUNT(*) as total FROM bookings WHERE listing_id = $1`, [listingId]);
        return {
            bookings: result.rows,
            total: parseInt(countResult.rows[0].total),
        };
 ...
 */
export const checkAvailability = async (listingId, checkInDate, checkOutDate, excludeBookingId) => {
    try {
        let query = `
 ...
 */
export const updateBookingStatus = async (bookingId, status, bookingStage) => {
    try {
 ...
 */
export const confirmBooking = async (bookingId, hostResponse) => {
    try {
       ...
       WHERE id = $4
       RETURNING *`, ['confirmed', 'confirmed', hostResponse || null, bookingId]);
        if (result.rows.length === 0) {
            throw new NotFoundError('Booking');
        }
        return result.rows[0];
 ...
 */
export const rejectBooking = async (bookingId, hostResponse) => {
    try {
        const result = await pool.query(`UPDATE bookings
       SET status = $1, host_response = $2, host_response_at = NOW(),
           updated_at = NOW()
       WHERE id = $3
       RETURNING *`, ['cancelled', hostResponse || null, bookingId]);
        if (result.rows.length === 0) {
            throw new NotFoundError('Booking');
        }
        return result.rows[0];
...
export const cancelBooking = async (bookingId, cancelledBy, cancellationReason) => {
    try {
       ...
       WHERE id = $4
       RETURNING *`, ['cancelled', cancelledBy, cancellationReason || null, bookingId]);
        if (result.rows.length === 0) {
            throw new NotFoundError('Booking');
        }
        return result.rows[0];
...
export const checkInBooking = async (bookingId) => {
    try {
       ...
       RETURNING *`, ['checked_in', bookingId]);
        if (result.rows.length === 0) {
...
export const checkOutBooking = async (bookingId) => {
    try {
       ...
       RETURNING *`, ['checked_out', 'completed', bookingId]);
        if (result.rows.length === 0) {
            throw new NotFoundError('Booking or booking not confirmed');
        }
        return result.rows[0];
 ...
 */
export const updatePaymentStatus = async (bookingId, paymentStatus, paymentIntentId) => {
    try {
        const result = await pool.query(`UPDATE bookings
       SET payment_status = $1, payment_intent_id = $2, updated_at = NOW()
       WHERE id = $3
       RETURNING *`, [paymentStatus, paymentIntentId || null, bookingId]);
        if (result.rows.length === 0) {
            throw new NotFoundError('Booking');
        }
        return result.rows[0];
 ...
 */
export const getListingBookingStats = async (listingId) => {
    try {
