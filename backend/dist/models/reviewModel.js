 ...
 */
export async function findReviewByBookingAndReviewer(bookingId, reviewerId) {
    const result = await pool.query(`SELECT * FROM reviews WHERE booking_id = $1 AND reviewer_id = $2`, [bookingId, reviewerId]);
    return result.rows[0] || null;
}
/**
 * Find all reviews for a listing
 */
export async function findReviewsByListingId(listingId, options = {}) {
    const { onlyPublished = true, limit = 20, offset = 0 } = options;
    let whereClause = 'WHERE listing_id = $1 AND review_type = $2';
    const params = [listingId, 'guest_to_host'];
    if (onlyPublished) {
        whereClause += ' AND is_published = true';
    }
    // Get total count
    const countResult = await pool.query(`SELECT COUNT(*) FROM reviews ${whereClause}`, params);
 ...
 */
export async function findReviewsByReviewerId(reviewerId, options = {}) {
    const { limit = 20, offset = 0 } = options;
    // Get total count
    const countResult = await pool.query(`SELECT COUNT(*) FROM reviews WHERE reviewer_id = $1`, [reviewerId]);
    const total = parseInt(countResult.rows[0].count);
    // Get reviews
    const result = await pool.query(`SELECT r.*, 
     ...
     ORDER BY r.created_at DESC
     LIMIT $2 OFFSET $3`, [reviewerId, limit, offset]);
    return {
        reviews: result.rows,
        total
    };
 ...
 */
export async function findReviewsByRevieweeId(revieweeId, options = {}) {
    ...
    // Get total count
    const countResult = await pool.query(`SELECT COUNT(*) FROM reviews r ${whereClause}`, params);
    const total = parseInt(countResult.rows[0].count);
    // Get reviews
    const result = await pool.query(`SELECT r.*, 
 ...
 */
export async function updateReviewResponse(reviewId, responseText) {
    const result = await pool.query(`UPDATE reviews 
     SET response_text = $1, 
         response_at = CURRENT_TIMESTAMP,
         updated_at = CURRENT_TIMESTAMP
     WHERE id = $2
     RETURNING *`, [responseText, reviewId]);
 ...
 */
export async function getListingAverageRatings(listingId) {
    const result = await pool.query(`SELECT 
     ...
     WHERE listing_id = $1 
       ...
       AND is_published = true`, [listingId]);
    return result.rows[0];
}
/**
 * Get host's average rating from all their listings
 */
export async function getHostAverageRating(hostId) {
    const result = await pool.query(`SELECT 
     ...
     WHERE l.host_id = $1 
       ...
       AND r.is_published = true`, [hostId]);
    return result.rows[0];
}
/**
 * Check if user can review a booking
 */
export async function canUserReviewBooking(bookingId, userId) {
    // Check if booking is completed
    const bookingResult = await pool.query(`SELECT status, guest_id, host_id FROM bookings WHERE id = $1`, [bookingId]);
    ...
    const reviewResult = await pool.query(`SELECT id FROM reviews WHERE booking_id = $1 AND reviewer_id = $2`, [bookingId, userId]);
