 ...
 */
export async function findPaymentTransactionsByBookingId(
  bookingId: string
): Promise<PaymentTransaction[]> {
  const result = await pool.query(
 ...
 */
export async function findPaymentTransactionsByHostId(
  hostId: string,
  options: {
...
export async function getHostEarningsSummary(hostId: string): Promise<{
  ...
  const result = await pool.query(
    `SELECT * FROM host_earnings_summary WHERE host_id = $1`,
    [hostId]
  );
 ...
 */
export async function getPendingCommissionsForHost(
  hostId: string
): Promise<{ transactions: PaymentTransaction[]; total_amount: number }> {
  const result = await pool.query(
    `SELECT pt.*, 
      b.check_in_date, b.check_out_date,
      l.title as listing_title
     ...
     ORDER BY pt.payment_due_date ASC NULLS LAST, pt.created_at ASC`,
    [hostId]
  );

  const total_amount = result.rows.reduce(
    (sum, tx) => sum + parseFloat(tx.commission_amount),
    0
  );
...
): Promise<{ commission_amount: number; net_amount: number }> {
  const result = await pool.query(
    `SELECT * FROM calculate_host_commission($1, $2)`,
    [bookingTotal, commissionRate]
  );
