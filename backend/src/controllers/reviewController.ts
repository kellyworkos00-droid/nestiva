 ...
 */
export async function getListingRatings(req: Request, res: Response): Promise<void> {
  try {
    const { listingId } = req.params;
 ...
 */
export async function getUserReviewsReceived(req: Request, res: Response): Promise<void> {
  try {
    const { userId } = req.params;
    const reviewType = req.query.reviewType as 'guest_to_host' | 'host_to_guest' | undefined;
    const limit = parseInt(req.query.limit as string) || 20;
    const offset = parseInt(req.query.offset as string) || 0;
 ...
 */
export async function getHostRating(req: Request, res: Response): Promise<void> {
  try {
 ...
 */
export async function canReviewBooking(req: Request, res: Response): Promise<void> {
  try {
    const userId = (req as any).user?.userId;
    if (!userId) {
      res.status(401).json(createErrorResponse('Unauthorized'));
 ...
 * GET /reviews/pending
 */
export async function getPendingReviews(req: Request, res: Response): Promise<void> {
  try {
    const userId = (req as any).user?.userId;
    if (!userId) {
      res.status(401).json(createErrorResponse('Unauthorized'));
