 ...
 */
export async function createCommissionForBooking(req: Request, res: Response): Promise<void> {
  try {
 ...
 */
export async function getHostTransactions(req: Request, res: Response): Promise<void> {
  try {
    const userId = (req as any).user?.userId;
    if (!userId) {
      res.status(401).json(createErrorResponse('Unauthorized'));
    ...
    const transactionType = req.query.type as any;
 ...
 */
export async function getHostEarnings(req: Request, res: Response): Promise<void> {
  try {
    const userId = (req as any).user?.userId;
    if (!userId) {
      res.status(401).json(createErrorResponse('Unauthorized'));
 ...
 */
export async function getPendingCommissions(req: Request, res: Response): Promise<void> {
  try {
    const userId = (req as any).user?.userId;
    if (!userId) {
      res.status(401).json(createErrorResponse('Unauthorized'));
...
export async function payCommission(req: Request, res: Response): Promise<void> {
  try {
    ...
    const transaction = await paymentService.processCommissionPayment(
      transactionId,
      userId,
      {
        payment_method,
 ...
 */
export async function calculateCommission(req: Request, res: Response): Promise<void> {
  try {
    ...
    const calculation = await paymentService.calculateCommissionPreview(
      parseFloat(booking_amount)
    );
 ...
 */
export async function getCommissionRate(req: Request, res: Response): Promise<void> {
  try {
 ...
 */
export async function getPlatformRevenue(req: Request, res: Response): Promise<void> {
  try {
    const userId = (req as any).user?.userId;
    if (!userId) {
      res.status(401).json(createErrorResponse('Unauthorized'));
